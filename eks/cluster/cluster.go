...
func (ts *tester) updateClusterStatus(v wait_v2.ClusterStatus, desired string) {
	if v.Cluster == nil {
		if v.Error != nil {
	...
	ts.cfg.EKSConfig.Status.ClusterARN = aws_v2.ToString(v.Cluster.Arn)
	ts.cfg.EKSConfig.RecordStatus(fmt.Sprint(v.Cluster.Status))
	...
	if desired != eksconfig.ClusterStatusDELETEDORNOTEXIST &&
		...
		if v.Cluster.Endpoint != nil {
			ts.cfg.EKSConfig.Status.ClusterAPIServerEndpoint = aws_v2.ToString(v.Cluster.Endpoint)
		}

		if v.Cluster.Identity != nil &&
			v.Cluster.Identity.Oidc != nil &&
			v.Cluster.Identity.Oidc.Issuer != nil {
			ts.cfg.EKSConfig.Status.ClusterOIDCIssuerURL = aws_v2.ToString(v.Cluster.Identity.Oidc.Issuer)
		...
		if v.Cluster.CertificateAuthority != nil && v.Cluster.CertificateAuthority.Data != nil {
			ts.cfg.EKSConfig.Status.ClusterCA = aws_v2.ToString(v.Cluster.CertificateAuthority.Data)
		}
